...
class TestElementSymbolsVsRDKit:
    ...
    def test_element_symbols(self, name, smi, _n, _b):
        ...
        for i in range(rd_mol.GetNumAtoms()):
            ...
            assert ce_sym == rd_sym, \
                f"{name} atom {i}: CE={ce_sym!r} vs RDKit={rd_sym!r}"
...
class TestAromaticityVsRDKit:
    ...
    def test_aromatic_flags(self, smi):
        ...
        for i in range(rd_mol.GetNumAtoms()):
            ...
            assert ce_arom == rd_arom, \
                f"{smi} atom {i}: CE={ce_arom} vs RDKit={rd_arom}"
...
class TestBondTypesVsRDKit:
    ...
    @pytest.mark.parametrize("smi", ["CC", "C=C", "C#C", "C=O", "C#N",
                                      "c1ccccc1", "CC(=O)O", "C1CCCCC1"])
...
class TestRotatableBondsVsRDKit:
    ...
    @pytest.mark.parametrize("smi,expected_rk", [
        ("CC",           0),
        ("CCC",          0),
        ("CCCC",         1),
        ("CCCCC",        2),
...
class TestRdKitRoundTrip:
    ...
    def test_full_round_trip_atom_count(self, name, smi, _n, _b):
        ...
        rd  = to_rdkit(ce1)
        ce2 = from_rdkit(rd)
        assert ce2.num_atoms == ce1.num_atoms, \
            f"{name}: {ce1.num_atoms} → {ce2.num_atoms}"
...
class TestSimilarityOrderingVsRDKit:
    ...
    def test_closely_related_more_similar_than_unrelated(self):
        ...
        gen_fps = lambda m: AllChem.GetMorganFingerprintAsBitVect(m, 2, 2048)
        ...
        rk_order = DataStructs.TanimotoSimilarity(rd_fps[0], rd_fps[1]) > \
                   DataStructs.TanimotoSimilarity(rd_fps[0], rd_fps[2])
        ce_order = ce_mols[0].similarity(ce_mols[1]) > \
                   ce_mols[0].similarity(ce_mols[2])
