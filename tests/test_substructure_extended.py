...
class TestBasicSubstructure:
    def test_single_carbon_in_all_organics(self):
        ...
        for smi in aliphatic_targets:
            ...
            assert target.has_substruct_match(query), f"[C] not found in {smi}"
    ...
    def test_methanol_in_longer_alcohols(self):
        ...
        for smi in targets_yes:
            assert ro.parse_smiles(smi).has_substruct_match(query), f"CO not found in {smi}"
        for smi in targets_no:
            assert not ro.parse_smiles(smi).has_substruct_match(query), f"CO wrongly found in {smi}"
...
class TestSubstructureWithHeteroatoms:
    ...
    def test_carboxyl_group(self):
        ...
        for smi in positives:
            assert ro.parse_smiles(smi).has_substruct_match(query), f"Carboxyl not found in {smi}"

    def test_amide_fragment(self):
        query = ro.parse_smiles("C(=O)N")  # amide bond
        positives = ["CC(=O)N", "CC(=O)NC", "CC(=O)Nc1ccc(O)cc1"]
        for smi in positives:
            assert ro.parse_smiles(smi).has_substruct_match(query), f"Amide not found in {smi}"
...
class TestSubstructureLargerMolecules:
    ...
    def test_caffeine_contains_imidazole_fragment(self):
        ...
        n_count = sum(
            1 for i in range(caffeine.num_atoms) if caffeine.get_atom(i).atomic_number == 7
        )
...
class TestSubstructureNoBonds:
    ...
    def test_single_F_in_fluorobenzene(self):
        assert ro.parse_smiles("Fc1ccccc1").has_substruct_match(ro.parse_smiles("[F]"))

    def test_single_Cl_not_in_fluorobenzene(self):
        assert not ro.parse_smiles("Fc1ccccc1").has_substruct_match(ro.parse_smiles("[Cl]"))
